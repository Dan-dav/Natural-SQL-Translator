...
lin
  ...
  StSelect col fromlim pred order = mkImp (mkVP (mkVP D.display_V2 (mkNP col.np (mkAdv col.prep (mkNP fromlim pred)))) order) ;
  ...
  LikeBegins = P.mkV2 D.begin_V with_Prep ;
  -- LikeEnds = "ends with" ;
  LikeEnds = P.mkV2 D.end_V with_Prep ;
  -- LikeContains = "contains" ;
  LikeContains = D.contain_V2 ;
  ...
  -- N Adv -> Imp
  StDelete tab pred = mkImp D.delete_V2 (mkNP (mkNP all_Predet (mkNP tab)) pred) ;
  ...
  -- N NP -> Imp
  StInsert tab ins = mkImp (mkVP (mkVP D.add_V2 (mkNP (mkNP a_Det D.record_N) (mkAdv with_Prep ins))) (mkAdv to_Prep (mkNP tab))) ;
  ...
  -- ListNP -> NP
  IOnlyValMultiple vs = mkNP (mkNP the_Det D.value_N) (mkAdv P.noPrep (mkNP and_Conj vs)) ;
  ...
  -- N NP Adv -> Imp
  StUpdate tab upd pred = mkImp (mkVP (mkVP D.set_V2 upd) (mkAdv for_Prep (mkNP (mkNP all_Predet (mkNP tab)) pred))) ;
